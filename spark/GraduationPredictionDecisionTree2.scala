...
object GraduationPredictionDecisionTree2 {
  def main(args: Array[String]): Unit = {
    // 初始化 SparkSession
    val spark = SparkSession.builder()
      .appName("Graduation Management System - Decision Tree")
      .master("local[*]") // 本地模式
      .getOrCreate()
    ...
    val dt = new DecisionTreeClassifier()
      ...
      .setMaxDepth(5) // 决策树最大深度，可根据需要调整

    // 训练模型
    val dtModel = dt.fit(trainingData)

    // 保存模型到本地
    val modelPath = "C:/Users/73269/Desktop/ab/2024/大三下/Web系统设计/大作业/code/GraduationManagementSystem/web/mode/model2" // 替换为你想要保存模型的实际路径
    dtModel.write.overwrite().save(modelPath)
    println(s"Decision Tree Model saved to: $modelPath")
